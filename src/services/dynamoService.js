...
export async function listChatMessages({ livestream_id, limit = 100 }) {
  ...
  const messages = items
    .map((item) => {
      const aiResponse = item.aiResponse?.S ?? item.ai_response?.S ?? ''
      ...
      return {
        ...
        aiResponse,
        tandem_reply: aiResponse,
        flagged_important: Boolean(item.flaggedImportant?.BOOL ?? item.flagged_important?.BOOL),
        needs_verification: Boolean(item.needsVerification?.BOOL ?? item.needs_verification?.BOOL),
        agent3_reason: item.agent3Reason?.S ?? item.agent3_reason?.S ?? '',
        sender_type: item.senderType?.S ?? item.sender_type?.S ?? 'buyer',
        in_reply_to_conversation_id: item.inReplyToConversationId?.S ?? item.in_reply_to_conversation_id?.S ?? '',
        in_reply_to_buyer_username: item.inReplyToBuyerUsername?.S ?? item.in_reply_to_buyer_username?.S ?? '',
