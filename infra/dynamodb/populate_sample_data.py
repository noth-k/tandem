#!/usr/bin/env python3
"""Populate DynamoDB tables with sample electronics data."""

import os
import sys
import time
from datetime import datetime, timedelta

# Ensure the repo root is on sys.path when running this file directly.
REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if REPO_ROOT not in sys.path:
    sys.path.insert(0, REPO_ROOT)

from infra.dynamodb.client import (
    add_product_name,
    put_discount,
    put_message,
    put_product,
)


def iso_now(offset_seconds: int = 0) -> str:
    return (datetime.utcnow() + timedelta(seconds=offset_seconds)).isoformat(timespec='seconds') + 'Z'


def epoch_ms(offset_seconds: int = 0) -> int:
    return int((datetime.utcnow() + timedelta(seconds=offset_seconds)).timestamp() * 1000)


def main():
    products = [
        {
            'productId': 'ipad-pro-12-9',
            'name': 'iPad Pro 12.9"',
            'stockQty': 45,
            'price': 1399.00,
            'currency': 'SGD',
            'description': '12.9-inch Liquid Retina XDR display, M2 chip, 512GB storage.',
            'sku': 'BD-IPADI2-512',
            'platformIds': {'tiktok': 'BDIPP-01', 'shopee': 'BDIPP-01'},
            'tags': ['tablet', 'pro', 'apple'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'ipad-air',
            'name': 'iPad Air',
            'stockQty': 62,
            'price': 749.00,
            'currency': 'SGD',
            'description': '10.9-inch display, M1 chip, 256GB storage, space gray.',
            'sku': 'BD-IPADA1-256',
            'platformIds': {'tiktok': 'BDIPA-01', 'shopee': 'BDIPA-01'},
            'tags': ['tablet', 'midrange', 'apple'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'airpods-pro-2',
            'name': 'AirPods Pro 2',
            'stockQty': 85,
            'price': 249.00,
            'currency': 'SGD',
            'description': 'Active noise cancellation, adaptive transparency, MagSafe case.',
            'sku': 'BD-AP2P',
            'platformIds': {'tiktok': 'BDAP2-01', 'shopee': 'BDAP2-01'},
            'tags': ['audio', 'wireless', 'apple'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'macbook-air-m2',
            'name': 'MacBook Air M2',
            'stockQty': 28,
            'price': 1199.00,
            'currency': 'SGD',
            'description': '13.6-inch display, M2 chip, 16GB RAM, 512GB SSD.',
            'sku': 'BD-MBA2-512',
            'platformIds': {'tiktok': 'BDMBA-01', 'shopee': 'BDMBA-01'},
            'tags': ['laptop', 'apple', 'ultrabook'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'samsung-galaxy-tab-s9',
            'name': 'Samsung Galaxy Tab S9',
            'stockQty': 36,
            'price': 899.00,
            'currency': 'SGD',
            'description': '11-inch AMOLED display, Snapdragon processor, 256GB storage.',
            'sku': 'BD-SGST9',
            'platformIds': {'tiktok': 'BDGST9-01', 'shopee': 'BDGST9-01'},
            'tags': ['tablet', 'android', 'samsung'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'sony-wh-1000xm5',
            'name': 'Sony WH-1000XM5',
            'stockQty': 54,
            'price': 349.00,
            'currency': 'SGD',
            'description': 'Industry-leading noise cancellation wireless headphones.',
            'sku': 'BD-SWHXM5',
            'platformIds': {'tiktok': 'BDSWH-01', 'shopee': 'BDSWH-01'},
            'tags': ['audio', 'headphones', 'sony'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'apple-watch-series-9',
            'name': 'Apple Watch Series 9',
            'stockQty': 38,
            'price': 399.00,
            'currency': 'SGD',
            'description': '45mm GPS + Cellular, blood oxygen, ECG, road-ready fitness tracker.',
            'sku': 'BD-AW9-45',
            'platformIds': {'tiktok': 'BDAW9-01', 'shopee': 'BDAW9-01'},
            'tags': ['wearable', 'apple', 'fitness'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'nintendo-switch-oleo',
            'name': 'Nintendo Switch OLED',
            'stockQty': 41,
            'price': 349.00,
            'currency': 'SGD',
            'description': '7-inch OLED screen, enhanced audio, 64GB storage.',
            'sku': 'BD-NSOLED',
            'platformIds': {'tiktok': 'BDSWO-01', 'shopee': 'BDSWO-01'},
            'tags': ['gaming', 'nintendo', 'console'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'go-pro-hero-12',
            'name': 'GoPro HERO12',
            'stockQty': 29,
            'price': 499.00,
            'currency': 'SGD',
            'description': '5.3K video, HyperSmooth 6, waterproof action camera.',
            'sku': 'BD-GPH12',
            'platformIds': {'tiktok': 'BDGPH-01', 'shopee': 'BDGPH-01'},
            'tags': ['camera', 'action', 'gopro'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'dyson-v15',
            'name': 'Dyson V15 Vacuum',
            'stockQty': 17,
            'price': 699.00,
            'currency': 'SGD',
            'description': 'Cordless stick vacuum with laser dust detection and powerful suction.',
            'sku': 'BD-DV15',
            'platformIds': {'tiktok': 'BDD15-01', 'shopee': 'BDD15-01'},
            'tags': ['home', 'vacuum', 'dyson'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'ps5-standard',
            'name': 'PlayStation 5 Standard',
            'stockQty': 18,
            'price': 549.00,
            'currency': 'SGD',
            'description': 'Next-gen console with 4K gaming and immersive haptic feedback.',
            'sku': 'BD-PS5STD',
            'platformIds': {'tiktok': 'BDPS5-01', 'shopee': 'BDPS5-01'},
            'tags': ['gaming', 'console', 'sony'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'bose-qc45',
            'name': 'Bose QuietComfort 45',
            'stockQty': 50,
            'price': 329.00,
            'currency': 'SGD',
            'description': 'Noise cancelling over-ear headphones with balanced audio.',
            'sku': 'BD-BOSEQC45',
            'platformIds': {'tiktok': 'BDBOSE-01', 'shopee': 'BDBOSE-01'},
            'tags': ['audio', 'headphones', 'bose'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'kindle-paperwhite',
            'name': 'Kindle Paperwhite',
            'stockQty': 30,
            'price': 149.00,
            'currency': 'SGD',
            'description': 'Waterproof e-reader with a 6.8-inch glare-free display.',
            'sku': 'BD-KINDLEPW',
            'platformIds': {'tiktok': 'BDKINDLE-01', 'shopee': 'BDKINDLE-01'},
            'tags': ['ebook', 'reading', 'amazon'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
        {
            'productId': 'galaxy-s24',
            'name': 'Samsung Galaxy S24',
            'stockQty': 22,
            'price': 999.00,
            'currency': 'SGD',
            'description': 'Flagship Android phone with triple-camera and AI features.',
            'sku': 'BD-S24',
            'platformIds': {'tiktok': 'BDS24-01', 'shopee': 'BDS24-01'},
            'tags': ['phone', 'android', 'samsung'],
            'isActive': True,
            'createdAt': iso_now(-3600),
        },
    ]

    discounts = [
        {
            'productId': 'ipad-pro-12-9',
            'startAt': epoch_ms(-1800),
            'discountId': 'disc-ipad-pro-15',
            'discountValueAmount': 0,
            'discountValuePercent': 15,
            'endAt': epoch_ms(3600),
            'acceptedBySeller': True,
            'appliedBy': 'audio-parser',
            'createdAt': iso_now(-1800),
        },
        {
            'productId': 'airpods-pro-2',
            'startAt': epoch_ms(-900),
            'discountId': 'disc-airpods-10',
            'discountValueAmount': 0,
            'discountValuePercent': 10,
            'endAt': epoch_ms(5400),
            'acceptedBySeller': True,
            'appliedBy': 'audio-parser',
            'createdAt': iso_now(-900),
        },
        {
            'productId': 'galaxy-s24',
            'startAt': epoch_ms(-600),
            'discountId': 'disc-s24-bundle',
            'discountValueAmount': 50,
            'discountValuePercent': 5,
            'endAt': epoch_ms(7200),
            'acceptedBySeller': True,
            'appliedBy': 'audio-parser',
            'createdAt': iso_now(-600),
        },
    ]

    messages = [
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-120),
            'buyerId': 'buyer-001',
            'buyerUsername': 'tech_lover88',
            'buyerMessage': 'Is the iPad Pro 12.9 still available today?',
            'aiCategory': 'PRODUCT_QUERY',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-105),
            'buyerId': 'buyer-002',
            'buyerUsername': 'audiofan',
            'buyerMessage': 'Do the AirPods Pro 2 support ANC in low latency mode?',
            'aiCategory': 'FUNCTIONAL_QUERY',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-90),
            'buyerId': 'buyer-003',
            'buyerUsername': 'gadgetqueen',
            'buyerMessage': 'What is the best bundle price for the MacBook Air M2?',
            'aiCategory': 'PURCHASE_INTENT',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-75),
            'buyerId': 'buyer-004',
            'buyerUsername': 'mom_of_two',
            'buyerMessage': 'How long does the Dyson V15 battery last on eco mode?',
            'aiCategory': 'FUNCTIONAL_QUERY',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-60),
            'buyerId': 'buyer-005',
            'buyerUsername': 'gamer_ryan',
            'buyerMessage': 'Is PS5 stock limited today or do you have more arrivals this week?',
            'aiCategory': 'AVAILABILITY_QUERY',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-45),
            'buyerId': 'buyer-006',
            'buyerUsername': 'bundle_hunter',
            'buyerMessage': 'Do you have a phone + earbuds bundle for the Galaxy S24?',
            'aiCategory': 'PURCHASE_INTENT',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-30),
            'buyerId': 'buyer-007',
            'buyerUsername': 'kindle_reader',
            'buyerMessage': 'Can the Kindle Paperwhite be read in bright sunlight?',
            'aiCategory': 'PRODUCT_QUERY',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-20),
            'buyerId': 'buyer-008',
            'buyerUsername': 'travel_bug',
            'buyerMessage': 'Does the GoPro HERO12 have an external mic port?',
            'aiCategory': 'FUNCTIONAL_QUERY',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-10),
            'buyerId': 'buyer-009',
            'buyerUsername': 'cleaning_queen',
            'buyerMessage': 'Does the Dyson V15 come with a hard-floor attachment?',
            'aiCategory': 'PRODUCT_QUERY',
            'aiResponse': '',
        },
        {
            'conversationId': 'shopee-live-001',
            'messageTimestamp': iso_now(-5),
            'buyerId': 'buyer-010',
            'buyerUsername': 'apple_fan',
            'buyerMessage': 'What watch bands are included with the Apple Watch Series 9?',
            'aiCategory': 'PRODUCT_QUERY',
            'aiResponse': '',
        },
    ]

    print('Writing sample products...')
    for product in products:
        put_product(product['productId'], {k: v for k, v in product.items() if k != 'productId'})
        add_product_name(product['name'], product['productId'])

    print('Writing sample discounts...')
    for discount in discounts:
        product_id = discount['productId']
        start_at = discount['startAt']
        discount_item = {k: v for k, v in discount.items() if k not in ('productId', 'startAt')}
        put_discount(product_id, start_at, discount_item)

    print('Writing sample messages...')
    for message in messages:
        convo_id = message.pop('conversationId')
        ts = message.pop('messageTimestamp')
        put_message(convo_id, ts, message)

    print('Sample data population complete.')


if __name__ == '__main__':
    main()
